...
v2.Object = {
  ...
  extend: function(obj, props, safe) {
    ...
    for (var prop in props) {
      ...
      if (props.hasOwnProperty(prop) && typeof obj[prop] == 'undefined') {
        obj[prop] = props[prop];
      }
    }
...
v2.FormUtil = {
  /**
   * Get the value of any form field. Returns:
   *
   *   - value attribute for input elements with type text, hidden, submit,
   *     button, password
   *   - innerHTML string from textarea elements
   *   - value attribute for selected radio button in radio group, or null if
   *     none is selected
   *   - value attribute for checkboxes in the same group if any are selected,
   *     otherwise null
   *   - value attribute of selected option element for selects, null if none
   *     is selected
   */
  value: function(field) {
    ...
    if (field.options) {
      ...
      // Multi select box
      for (var i = 0, opt = null; (opt = field.options[i]); i++) {
        if (opt.selected) {
    ...
    if (field.type && field.type == 'radio') {
      var elements = v2.$$('input[type=radio][name=' + field.name + ']', field.form);

      for (var i = 0, radio; (radio = elements[i]); i++) {
        if (radio.checked) {
          return radio.value;
        }
      }
    ...
    if (field.type && field.type == 'checkbox') {
      var elements = v2.$$('input[type=checkbox].g_' + field.className.match(/\bg_([^\s]*)\b/)[1], field.form);
      var values = [];

      for (var i = 0, checkbox; (checkbox = elements[i]); i++) {
        if (checkbox.checked) {
